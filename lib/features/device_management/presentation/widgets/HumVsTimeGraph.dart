...
class HumVsTimeGraph extends StatefulWidget {
  ...
  @override
  State<HumVsTimeGraph> createState() => _HumVsTimeGraphState();
}
...
List<List<FlSpot>> historicalCycles = [];  // New list to store historical cycles
  ...
  Future<void> _initializeHive() async {
    spotBox = await Hive.openBox('humidityData');
    cycleBox = await Hive.openBox('humidityMCycleData');  // New box for completed cycles
    _loadData();
    // clearSpotBox();
...
void _startPeriodicCheck() {
    _timer = Timer.periodic(const Duration(seconds: 5), (timer) {
      _generateHumidityData(sensorData); // Check data periodically
    });
  }
...
//mqttService.isDataReceived();
  ...
  void _generateHumidityData(Map<String, dynamic> sensorData) {
  ...
  Widget build(BuildContext context) {
    return Consumer<DeviceManager>(
      builder: (context, deviceManager, child) {        final sensorData = deviceManager.sensorData;
        _generateHumidityData(sensorData);
                  ...
                  LineChartData(
                    ...
                    lineBarsData: [
                      LineChartBarData(
                        spots: visibleSpots,
                        isCurved: true,
                        color: Colors.blue,                        belowBarData: BarAreaData(
                          show: true,
